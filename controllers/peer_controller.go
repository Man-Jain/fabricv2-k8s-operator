...
// +kubebuilder:rbac:groups=fabric.hyperledger.org,resources=peers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=fabric.hyperledger.org,resources=peers/status,verbs=get;update;patch

func (r *PeerReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	ctx := context.Background()
	reqLogger := r.Log.WithValues("peer", req.NamespacedName)
	...
	if err != nil && errors.IsNotFound(err) && !errors.IsAlreadyExists(err) {
		...
		if err != nil {
			reqLogger.Error(err, "Failed to create Fabric Peer secrets")
			return ctrl.Result{}, err
		}
...
func (r *PeerReconciler) newSecretForCR(cr *fabricv1alpha1.Peer, req ctrl.Request) *corev1.Secret {
	secret := &corev1.Secret{}
...
func (r *PeerReconciler) newServiceForCR(cr *fabricv1alpha1.Peer, req ctrl.Request) *corev1.Service {
...
func (r *PeerReconciler) newSTSForCR(cr *fabricv1alpha1.Peer, secret *corev1.Secret, req ctrl.Request) *appsv1.StatefulSet {
	...
	sts.Spec.Template.Spec.Containers[0].Command = []string{"peer", "node", "start"}
	...
	builderVolume := corev1.Volume{
		Name: req.Name + "-builder-files",
		VolumeSource: corev1.VolumeSource{
			HostPath: &corev1.HostPathVolumeSource{
				Path: cr.Spec.BuilderFileLoc,
			},
		},
	}

	corePeerFile := corev1.Volume{
		Name: req.Name + "-core-file",
		VolumeSource: corev1.VolumeSource{
			HostPath: &corev1.HostPathVolumeSource{
				Path: cr.Spec.CorePeerFile,
			},
		},
	}

	sts.Spec.Template.Spec.Volumes = append(sts.Spec.Template.Spec.Volumes, *mspVolume, *tlsVolume, builderVolume, corePeerFile)
	...
	builderVolumeMount := corev1.VolumeMount{
		Name:      req.Name + "-builder-files",
		MountPath: "/builders/external/bin",
	}

	corePeerFileVolumeMount := corev1.VolumeMount{
		Name:      req.Name + "-core-file",
		MountPath: "/etc/hyperledger/fabric/core.yaml",
	}

	sts.Spec.Template.Spec.Containers[0].VolumeMounts = append(sts.Spec.Template.Spec.Containers[0].VolumeMounts, mspVolumeMount, tlsVolumeMount, builderVolumeMount, corePeerFileVolumeMount)
...
func (r *PeerReconciler) mountSecretsToSTS(cr *fabricv1alpha1.Peer, secret *corev1.Secret, req ctrl.Request) (*corev1.Volume, *corev1.Volume) {
